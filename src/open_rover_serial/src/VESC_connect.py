...
class Vesc:
    ...
    def setandmonitorPWM(self, duty):
        Packet(8, PacketID.SET_DUTY, Scale.NONE, 32, duty, Scale.E5).send(self.left_back)
        sleep(0.3)
        ...
        if self.left_back.in_waiting > 5:
            ...
            if self.generalpacket.process_buffer(converted):
                self.left_back.process_packet(self.generalpacket.goodpacket)
                self.print_mc_data()
